...
In potential games, the best response learning rule converges to a Nash equilibrium of the game. However, this Nash equilibrium need not be efficient. For example, recall the game defined in Example~\ref{e:interdependent utilities}, with the agent utility functions below.
...
Mathematically, the best response process defines a Markov chain over the joint action space. The Markov chain associated with the best response learning rule for the example above is:

\todo[inline]{add figure here}


Log-linear learning belongs to the class of noisy best response learning rules: an updating agent selects a best response action the majority of the time, but occasionally selects suboptimally.
